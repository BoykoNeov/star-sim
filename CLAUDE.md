...
- **Pure JS helpers are unit-tested; everything that draws is checked by screenshot.**
  ...
  `gravdark` `classify` `reddening` `controls` `framebudget`) under `node --test` — no bundler, no npm, in CI.
...
`src/{main,star,hr,comp,lane,structure,roche,spectrum,sed,scale,classify,color,canvas,controls,layout,tooltip,sn,hz,hzhist,reddening,cmd,seismo,framebudget}.js`.
...
**`frontend/tests/`** — `node --test` over the eight DOM-free helpers, asserting the
