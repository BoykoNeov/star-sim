...
- [Visual & performance pass](star-sim-visual-performance.md) — the 2026-09-05/06 measured passes: the per-frame WebGL realloc at DPR ≥ 1.5, the Planck integral dominating the age scrub (exact-key memo), the pre-warm thread; then V1b's reserved-floor raise and its two traps (a floor owes the panel's bottom padding; size at the regime's NARROWEST width, 481 not 512); then V6's last two floors and the rule for which mechanism to use — sweep each panel over ITS OWN controls, and a note that is last in its panel is absorbed by the panel floor while one with siblings below it needs its own reserve.
