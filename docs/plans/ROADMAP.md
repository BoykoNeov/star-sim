...
| Merger time / inspiral clock for a classified DCO | idea | All that is left of the DCO gap: the pair is now **drawn** (the endpoint frame shipped 2026-09-06, see `SHIPPED.md`), but nothing says *when* it merges. Needs natal kicks — two prescriptions deep — so the pre-collapse orbit POSYDON hands over is not enough on its own. The kick-free Blaauw test already lives in `posydon_co.py` as an unbinding **gate**; a merger time would mean surfacing the post-SN orbit it implies, which is the part that needs the kick. | `tempered-lineage-inspiral.md`; memory `star-sim-dco-endpoint-view.md` |
