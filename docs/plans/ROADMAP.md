...
| Endgame Chunk 5 — WR 3D wind shader | planned | Optically-thick-wind look (radial outflow, emission-line glow); replaces the Chunk-4 smooth-sphere placeholder; closes the on-hand-data endgame work. | `smoldering-cinder-gateway.md` §Chunk 5 |
...
3. **Endgame Chunks 4–5** — the full WR/WD gateway on data already on disk (backend Chunk 1 + frontend Chunks 2 & 3 ✅ + **Chunk 4 ✅** = the WR mode shell + HR + stripped-surface WN→WC→WO composition + WN/WC/WO subtype; next is **Chunk 5 — the WR 3D wind shader**, which replaces Chunk 4's smooth-sphere placeholder and closes the on-hand-data endgame work).
