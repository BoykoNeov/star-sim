...
|---|---|---|---|
| Sun-residual tooltip | planned | One readout string: "MIST v2.5's 1 M☉ model is 7 % over-luminous at solar age; the grid's Sun sits near [Fe/H] +0.07." Frontend-only. | `science-hurdles.md` §1.2 |
| ECSN-regime caption (6.5–8 M☉) | planned | Between the last WD node and the first SN node the fate is genuinely uncertain (super-AGB / electron-capture); say so instead of a crisp WD/SN flip. Frontend-only; the boundary masses already come with `/endgame?meta=1`. | `science-hurdles.md` §2 |
| He-ignition-cliff caption | sketched | When the marker is within ±0.15 M☉ of the grid's He-flash boundary *and* in CHeB: "interpolated across the He-ignition transition; the real loop is sharper." Needs the boundary served once. | `science-hurdles.md` §1.3 |
| Rossby-flavoured `activity` proxy | sketched | Replace the pure Teff ramp with `f(Teff)·g(P_rot/τ_conv)` from the served `v_rot_kms` + R and the SED panel's gyrochronology period; closes the last spec §11 question; stays labeled evocative. | `science-hurdles.md` §1.6 |
