...
3. **Endgame Chunks 4–5** — the full WR/WD gateway on data already on disk (backend Chunk 1 + frontend Chunks 2 & 3 ✅ done: gateway + WD mode shell + the WD-correct shader & structure panel; next is the **WR branch** — Chunk 4 WR mode shell + HR + stripped-surface composition, then Chunk 5 the WR wind shader).
